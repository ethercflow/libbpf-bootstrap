...
int main(int argc, char **argv)
{
	...
	if (env.count) {
		obj->links.irq_handler_entry1 =
			bpf_program__attach(obj->progs.irq_handler_entry1);
		err = libbpf_get_error(obj->links.irq_handler_entry1);
		if (err) {
			fprintf(stderr,
		...
		obj->links.irq_handler_entry2 =
			bpf_program__attach(obj->progs.irq_handler_entry2);
		err = libbpf_get_error(obj->links.irq_handler_entry2);
		if (err) {
			fprintf(stderr,
				"failed to attach irq_handler_entry: %s\n",
				strerror(err));
		}
		obj->links.irq_handler_exit =
			bpf_program__attach(obj->progs.irq_handler_exit);
		err = libbpf_get_error(obj->links.irq_handler_exit);
		if (err) {
			fprintf(stderr,
